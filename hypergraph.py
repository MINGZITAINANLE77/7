...
def load_data():
    ...
    with open('ml-1m/users.dat', 'r', encoding='ISO-8859-1') as f:
        for line in f:
            ...
            users[user_id] = {'gender': gender, 'age': age, 'occupation': occupation, 'zip_code': zip_code}
    ...
    with open('ml-1m/ratings.dat', 'r', encoding='ISO-8859-1') as f:
        for line in f:
            ...
            ratings.append({'user_id': user_id, 'movie_id': movie_id, 'rating': rating, 'timestamp': timestamp})

    return users, movies, ratings

# 构建知识图谱
def construct_kg(users, movies, ratings):
    ...
    for rating in ratings:
        ...
        # 添加用户节点和电影节点
        if user_id not in kg:
            kg[user_id] = []
        kg[user_id].append((movie_id, 'rated'))
        if movie_id not in kg:
            kg[movie_id] = []
        kg[movie_id].append((user_id, 'rated_by'))
        # 添加评分节点
        rating_node_id = 'rating_' + user_id + '_' + movie_id
        if rating_node_id not in kg:
            kg[rating_node_id] = []
        kg[rating_node_id].append((user_id, 'rated_by'))
        kg[rating_node_id].append((movie_id, 'rated'))
        # 添加边权重
        kg[user_id][-1] = (kg[user_id][-1][0], kg[user_id][-1][1], rating_value)
        kg[movie_id][-1] = (kg[movie_id][-1][0], kg[movie_id][-1][1], rating_value)
        kg[rating_node_id].append((user_id, 'rated'))
        kg[rating_node_id][-1] = (movie_id, 'rated_by', rating_value)
...
class UserSequenceGraph:
    def __init__(self, rating_data: np.ndarray, min_sequence_length: int = 5) -> None:
        self.min_sequence_length = min_sequence_length
        self.user_item_dict = self._create_user_item_dict(rating_data)
        self.sequence_graph = self._create_sequence_graph()

    def _create_user_item_dict(self, rating_data: np.ndarray) -> Dict[int, List[Tuple[int, float]]]:
        # 构建用户-物品字典
        user_item_dict = defaultdict(list)
        for user_id, item_id, rating, timestamp in rating_data:
            user_item_dict[user_id].append((item_id, timestamp))
        # 按照时间戳排序
        for user_id in user_item_dict:
            user_item_dict[user_id].sort(key=lambda x: x[1])
        return user_item_dict

    def _create_sequence_graph(self) -> Dict[int, List[Tuple[int, float]]]:
        # 构建序列图
        sequence_graph = {}
        for user_id in tqdm(self.user_item_dict):
            item_sequence = self._create_item_sequence(self.user_item_dict[user_id])
            if len(item_sequence) >= self.min_sequence_length:
                sequence_graph[user_id] = item_sequence
        return sequence_graph

    def _create_item_sequence(self, user_items: List[Tuple[int, float]]) -> List[Tuple[int, float]]:
        # 构建用户的物品序列
        item_sequence = []
        for item_id, timestamp in user_items:
            if len(item_sequence) > 0 and item_id == item_sequence[-1][0]:
                # 忽略用户连续评分同一部电影的情况
                continue
            item_sequence.append((item_id, timestamp))
...
class EntityAlignment(nn.Module):
    def __init__(self, num_entities_1, num_entities_2, entity_dim, similarity_method):
        super(EntityAlignment, self).__init__()
        self.entity_embeddings_1 = nn.Embedding(num_entities_1, entity_dim)
        self.entity_embeddings_2 = nn.Embedding(num_entities_2, entity_dim)
        self.linear_layer_1 = nn.Linear(entity_dim, entity_dim)
        self.linear_layer_2 = nn.Linear(entity_dim, entity_dim)
        self.similarity_method = similarity_method

    def forward(self, entities_1, entities_2):
        embeddings_1 = self.entity_embeddings_1(entities_1)
        embeddings_2 = self.entity_embeddings_2(entities_2)
        embeddings_1 = self.linear_layer_1(embeddings_1)
        embeddings_2 = self.linear_layer_2(embeddings_2)
        if self.similarity_method == "cosine_similarity":
            scores = self.cosine_similarity(embeddings_1, embeddings_2)
        return scores

    def cosine_similarity(self, embeddings_1, embeddings_2):
        norm_1 = embeddings_1.norm(dim=1, keepdim=True)
        norm_2 = embeddings_2.norm(dim=1, keepdim=True)
        dot_product = torch.sum(embeddings_1 * embeddings_2, dim=1, keepdim=True)
        similarity = dot_product / (norm_1 * norm_2)
        return similarity
  
    entity_align = EntityAlignment(num_entities_1=num_users, num_entities_2=num_movies, entity_dim=32, similarity_method='cosine_similarity')

user_entity_embeddings = all_entity_embeddings[:num_user_entities]
knowledge_graph_user_entity_embeddings = all_entity_embeddings[num_user_entities:]

user_scores = entity_align(torch.LongTensor(user_entity_embeddings), 
                           torch.LongTensor(knowledge_graph_user_entity_embeddings)).detach().numpy()
best_user_matches = np.argmax(user_scores, axis=1)



 # 获取二部图中每个电影节点和知识图谱中最相似的电影节点
movie_entity_embeddings = all_entity_embeddings[num_user_entities:]
knowledge_graph_movie_entity_embeddings = all_entity_embeddings[num_user_entities + num_movie_entities:]

movie_scores = entity_align(torch.LongTensor(movie_entity_embeddings), 
                            torch.LongTensor(knowledge_graph_movie_entity_embeddings)).detach().numpy()
best_movie_matches = np.argmax(movie_scores, axis=1)

# 对齐二部图和知识图谱中的实体
aligned_user_entities = []
for i, user_id in enumerate(user_entities):
    best_match = knowledge_graph_user_entity_embeddings[best_user_matches[i]]
    if np.array_equal(user_entity_embeddings[i], best_match):
        aligned_user_entities.append(user_id)
    else:
        aligned_user_entities.append(-1)  # 如果匹配不上，则标记为 -1

aligned_movie_entities = []
for i, movie_id in enumerate(movie_entities):
    best_match = knowledge_graph_movie_entity_embeddings[best_movie_matches[i]]
    if np.array_equal(movie_entity_embeddings[i], best_match):
        aligned_movie_entities.append(movie_id)
    else:
        aligned_movie_entities.append(-1)  # 如果匹配不上，则标记为 -1


user_entity_embeddings = all_entity_embeddings[:num_user_entities]
sequence_graph_user_entity_embeddings = all_entity_embeddings[num_user_entities:num_user_entities+num_sequence_entities]
user_scores = entity_alignment(torch.LongTensor(user_entity_embeddings),
torch.LongTensor(sequence_graph_user_entity_embeddings)).detach().numpy()
best_user_matches = np.argmax(user_scores, axis=1)

#获取二部图中每个电影节点和序列图中最相似的电影节点
movie_entity_embeddings = all_entity_embeddings[num_user_entities+num_sequence_entities:]
sequence_graph_movie_entity_embeddings = all_entity_embeddings[num_user_entities+num_sequence_entities+num_movie_entities:]
movie_scores = entity_alignment(torch.LongTensor(movie_entity_embeddings),
torch.LongTensor(sequence_graph_movie_entity_embeddings)).detach().numpy()
best_movie_matches = np.argmax(movie_scores, axis=1)

#对齐二部图和序列图中的实体
aligned_user_entities = []
for i, user_id in enumerate(user_entities):
    best_match = sequence_graph_user_entity_embeddings[best_user_matches[i]]
if np.array_equal(user_entity_embeddings[i], best_match):
    aligned_user_entities.append(user_id)
else:
    aligned_user_entities.append(-1) # 如果匹配不上，则标记为 -1
...
aligned_movie_entities = []
for i, movie_id in enumerate(movie_entities):
    best_match = sequence_graph_movie_entity_embeddings[best_movie_matches[i]]
if np.array_equal(movie_entity_embeddings[i], best_match):
    aligned_movie_entities.append(movie_id)
else:
    aligned_movie_entities.append(-1) # 如果匹配不上，则标记为 -1

print("Aligned User Entities:", aligned_user_entities)
print("Aligned Movie Entities:", aligned_movie_entities)
...
# 构建行为超边
    for  row in ratings.iterrows():
        user_id = f'user:{row.userId}'
        item_id = f'item:{row.movieId}'
        rating = row.rating
    ...
    # 添加电影节点
    G.add_node(item_id, type='item', title=movie_dict[row.movieId])

    # 添加用户行为节点
    G.add_node(f'action:rating', type='action')

    # 添加超边
    G.add_edge(user_id, f'action:rating')
    G.add_edge(f'action:rating', item_id, weight=rating)

    #属性超边


# 读取movies.dat文件
movies_df = pd.read_csv('ml-1m/movies.dat', sep='::', engine='python', header=None, names=['movie_id', 'title', 'genres'])

# 解析电影类型信息
genres_list = []
for genres_str in movies_df['genres'].values:
    genres_list.append(genres_str.split('|'))
genres_df = pd.DataFrame({'movie_id': movies_df['movie_id'].values, 'genre': genres_list})
genres_df = pd.get_dummies(genres_df.explode('genre'), columns=['genre'], prefix='genre')
genres_df = genres_df.groupby('movie_id').sum().reset_index()

# 解析导演和演员信息
directors_list = []
actors_list = []
for movie_id in movies_df['movie_id'].values:
    movie_file_path = f'ml-1m/movies/{movie_id}.dat'
    with open(movie_file_path, 'r', encoding='ISO-8859-1') as f:
        lines = f.readlines()
        for line in lines:
            if line.startswith('Director'):
                ...
                actors = line.split(': ')[1].strip().split(', ')
                actors_list.append(actors)
        if not lines:
            directors_list.append('')
            actors_list.append([])
            
actors_df = pd.DataFrame({'movie_id': movies_df['movie_id'].values, 'actor': actors_list})
actors_df = actors_df.explode('actor').reset_index(drop=True)
actors_df = pd.get_dummies(actors_df, columns=['actor'], prefix='actor')
actors_df = actors_df.groupby('movie_id').sum().reset_index()

directors_df = pd.DataFrame({'movie_id': movies_df['movie_id'].values, 'director': directors_list})
directors_df = pd.get_dummies(directors_df, columns=['director'], prefix='director')
directors_df = directors_df.groupby('movie_id').sum().reset_index()

# 合并所有属性信息
attributes_df = pd.merge(genres_df, actors_df, on='movie_id', how='outer')
attributes_df = pd.merge(attributes_df, directors_df, on='movie_id', how='outer')
attributes_df.fillna(0, inplace=True)

# 将属性信息转换成超边信息
num_movies = len(movies_df)
num_genres = genres_df.shape[1] - 1  # 去掉movie_id列
num_actors = actors_df.shape[1] - 1  # 去掉movie_id列
num_directors = directors_df.shape[1] - 1  # 去掉movie_id列
attribute_hyperedges = np.zeros((num_movies, num_genres + num_actors + num_directors))
attribute_hyperedges[:, :num_genres] = genres_df.iloc[:, 1:].values

attribute_hyperedges[:, num_genres+num_actors:num_genres+num_actors+num_directors] = directors_df.iloc[:, 1:].values
attribute_hyperedges = normalize(attribute_hyperedges, axis=1)
    ...
    def add_hyperedges_from_feature_kNN(self, feature: torch.Tensor, k: int, group_name: str = "main"):
        ...
        #喀喀喀卡卡卡卡
    def add_attribute_hyperedges(self, attribute_hyperedges: np.ndarray, k: int, group_name: str = "attribute"):
        e_list = []
        for i in range(attribute_hyperedges.shape[0]):
            # 获取最近的 k 个邻居
            knn_indices = NearestNeighbors(n_neighbors=k, metric="cosine").fit(attribute_hyperedges).kneighbors([attribute_hyperedges[i]])[1][0]
            e_list.append((i, knn_indices.tolist()))
        self.add_hyperedges(e_list, group_name=group_name)
